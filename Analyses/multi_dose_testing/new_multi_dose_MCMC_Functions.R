...
prior_function <- function(parameters_vector, informative_prior) {
  ...
  k_abs <- parameters_vector[, "k_abs"]
  bioavailability <- parameters_vector[, "bioavailability"]
  sigma <- parameters_vector[, "sigma"]
  k_alb <- parameters_vector[, "k_alb"]
  k_alb_so_init <- parameters_vector[, "k_alb_so_init"]
  k_env <- parameters_vector[, "k_env"]
  Emax <- parameters_vector[, "Emax"]
  EC50 <- parameters_vector[, "EC50"]
  ...
  if (informative_prior == TRUE) {
    ...
    k_alb_so_init_prior_value <- dtmvnorm(k_alb_so_init, mean = 0.2, sigma = 0.04, lower = 0, log = TRUE)
    k_env_prior_value <- dtmvnorm(k_env, mean = 0, sigma = 5, lower = 0, log = TRUE)
    Emax_prior_value <- dtmvnorm(Emax, mean = 0, sigma = 5, lower = 0, log = TRUE)
    EC50_prior_value <- dtmvnorm(EC50, mean = 0, sigma = 5, lower = 0, upper = Emax, log = TRUE)
  } else {
    k_abs_prior_value <- dtmvnorm(k_abs, mean = 5, sigma = 7, lower = 0.1, log = TRUE)
    bioavailability_prior_value <- dtmvnorm(bioavailability, mean = 0.005, sigma = 0.01, lower = 0, log = TRUE)
    ...
    k_alb_so_init_prior_value <- dtmvnorm(k_alb_so_init, mean = 0.20, sigma = 0.40, lower = 0, log = TRUE)
    k_env_prior_value <- dtmvnorm(k_env, mean = 0, sigma = 5, lower = 0, log = TRUE)
    Emax_prior_value <- dtmvnorm(Emax, mean = 0, sigma = 15, lower = 0, log = TRUE)
    EC50_prior_value <- dtmvnorm(EC50, mean = 0, sigma = 15, lower = 0, upper = Emax, log = TRUE)
  }

  prior_output <- k_abs_prior_value + bioavailability_prior_value + sigma_prior_value + k_alb_so_init_prior_value + 
                  k_alb_prior_value + k_env_prior_value + Emax_prior_value + EC50_prior_value
  return(unname(prior_output))
  
}

#Likelihood Function
# DO WE WANT ENZYME TO INFLUENCE HALF LIFE OF ALB-SO AND ALB OR JUST ALB-SO.
loglikelihood_function <- function(parameters_vector, Alb_data, Alb_SO_data, metabolite_availability, 
                                   model_instance, dose_times, dose_amounts) {
  
  for (i in 1:length(dose_times)) {
    if (i == 1) {
      mod <- Albendazole_PK_Model(k_abs = parameters_vector[, "k_abs"], sigma = parameters_vector[, "sigma"], 
                                  k_alb = parameters_vector[, "k_alb"], k_alb_so_init = parameters_vector[, "k_alb_so_init"], 
                                  k_env = parameters_vector[, "k_env"], EC50 = parameters_vector[, "EC50"], Emax = parameters_vector[, "Emax"],
...
# Posterior Function
posterior_function <- function(parameters_vector, Alb_data, Alb_SO_data, metabolite_availability, model_instance, dose_times, dose_amounts, informative_prior){
  posterior <- loglikelihood_function(parameters_vector, Alb_data, Alb_SO_data, metabolite_availability, model_instance, dose_times, dose_amounts) + 
               prior_function(parameters_vector, informative_prior)
  return(posterior)
}
...
# accepted whether or not the most recent parameter proposal was accepted
# i the iteration number
# current_sf the current scaling factor
# previous_mu running average of the MCMC parameters
# current_parameters current parameters
# current_covariance_matrix current covariance matrix
# required_acceptance_ratio required acceptance ratio
...
MCMC_running <- function(number_of_iterations, parameters_vector, sd_proposals, start_covariance_adaptation, 
                         Alb_data, Alb_SO_data, metabolite_availability, model_instance, dose_times, dose_amounts, 
                         output, informative_prior, reparam, refresh) {
  ...
  scaling_factor_storage <- vector(length = number_of_iterations - start_covariance_adaptation + 1)
  ...
  MCMC_output[1, ] <- c(parameters_vector["k_abs"], parameters_vector["bioavailability"], parameters_vector["sigma"], 
                        parameters_vector["k_alb"], parameters_vector["k_alb_so_init"], 
                        parameters_vector["k_env"], parameters_vector["EC50"], parameters_vector["Emax"])
  colnames(MCMC_output) = c("k_abs", "bioavailability", "sigma", "k_alb", "k_alb_so_init", "k_env", "EC50", "Emax")
  ...
  current_posterior <- posterior_function(reparam_current_parameter_values, Alb_data, Alb_SO_data, metabolite_availability, model_instance, dose_times, dose_amounts, informative_prior)
  ...
  for (i in 1:number_of_iterations){
    ...
    colnames(reparam_proposed_parameter_values) <- c("k_abs", "bioavailability", "sigma", "k_alb", "k_alb_so_init", "k_env", "EC50", "Emax")
    proposed_posterior <- posterior_function(reparam_proposed_parameter_values, Alb_data, Alb_SO_data, metabolite_availability, model_instance, dose_times, dose_amounts, informative_prior)
    ...
    covariance_matrix_storage[[i]] <- scaling_factor  * sigma
    scaling_factor_storage[i-start_covariance_adaptation+1] <- scaling_factor
  ...
  list[["Scaling Factor"]] <- scaling_factor_storage
  return(list)
}
