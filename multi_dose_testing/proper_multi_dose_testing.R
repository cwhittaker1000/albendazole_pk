...
source("multi_dose_testing/multi_dose_MCMC_Functions.R")

# Generate fake data 
dose_times <- c(0, 10, 20, 30)
dose_amounts <- c(400000, 400000, 400000, 400000)
parameters_vector <- c(0.5, 0.001, 15, 0.2, 0.02, 0.050, 0.05)
for (i in 1:length(dose_times)) {
  if (i == 1) {
    mod <- Albendazole_PK_Model(k_abs = parameters_vector[1], sigma = parameters_vector[3], k_alb = parameters_vector[4], 
                                k_alb_so_init = parameters_vector[5], auto_ind = parameters_vector[6], k_env = parameters_vector[7],
...
n_mcmc <- 50000
names(parameters_vector) <- c("k_abs", "bioavailability", "sigma", "k_alb", "k_alb_so_init", "auto_ind", "k_env")
reparam <- c(10, 1000, 0.1, 10, 100, 100, 100)
...
hist(mcmc_output[10000:20000, 1]/reparam[1])
abline(v = parameters_vector[1], col = "red", lwd = 5)

hist(mcmc_output[10000:20000, 2]/reparam[2])
abline(v = parameters_vector[2], col = "red", lwd = 5)

hist(mcmc_output[10000:20000, 3]/reparam[3])
abline(v = parameters_vector[3], col = "red", lwd = 5)

hist(mcmc_output[10000:20000, 4]/reparam[4])
abline(v = parameters_vector[4], col = "red", lwd = 5)

hist(mcmc_output[10000:20000, 5]/reparam[5])
abline(v = parameters_vector[5], col = "red", lwd = 5)

hist(mcmc_output[10000:20000, 6]/reparam[6])
abline(v = parameters_vector[6], col = "red", lwd = 5)

hist(mcmc_output[10000:20000, 7]/reparam[7])
