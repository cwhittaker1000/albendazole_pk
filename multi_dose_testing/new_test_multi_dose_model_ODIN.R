Albendazole_PK_Model <- odin::odin({
  ...
  deriv(Enzyme_conc) <-  (k_env * (Emax * Blood_Alb_SO)/(EC50 + Blood_Alb_SO)) - k_env * Enzyme_conc
  k_alb_so <- k_alb_so_init * (1 + Enzyme_conc)
  ...
  ## Outputs
  output(k_alb_so) <- TRUE
  x <- (Emax * Blood_Alb_SO)/(EC50 + Blood_Alb_SO)
  output(x) <- TRUE
})
