...
prior_function <- function(parameters_vector, informative_prior) {
  ...
  auto_ind <- parameters_vector[, "auto_ind"]
  ...
  if (informative_prior == TRUE) {
    ...
    auto_ind_prior_value <- dtmvnorm(auto_ind, mean = 0, sigma = 5, lower = 0, log = TRUE)
  ...
  prior_output <- k_abs_prior_value + bioavailability_prior_value + sigma_prior_value + k_alb_so_init_prior_value + 
                  k_alb_prior_value + auto_ind_prior_value + k_env_prior_value
  ...
  for (i in 1:length(dose_times)) {
    if (i == 1) {
      mod <- Albendazole_PK_Model(k_abs = parameters_vector[, "k_abs"], sigma = parameters_vector[, "sigma"], 
                                  ...
                                  auto_ind = parameters_vector[, "auto_ind"], k_env = parameters_vector[, "k_env"],
...
MCMC_running <- function(number_of_iterations, parameters_vector, sd_proposals, start_covariance_adaptation, 
  ...
  MCMC_output[1, ] <- c(parameters_vector["k_abs"], parameters_vector["bioavailability"], parameters_vector["sigma"], 
                        parameters_vector["k_alb"], parameters_vector["k_alb_so_init"], parameters_vector["auto_ind"], parameters_vector["k_env"])
  colnames(MCMC_output) = c("k_abs", "bioavailability", "sigma", "k_alb", "k_alb_so_init", "auto_ind", "k_env")
  ...
  for (i in 1:number_of_iterations){
    ...
    colnames(reparam_proposed_parameter_values) <- c("k_abs", "bioavailability", "sigma", "k_alb", "k_alb_so_init", "auto_ind", "k_env")
